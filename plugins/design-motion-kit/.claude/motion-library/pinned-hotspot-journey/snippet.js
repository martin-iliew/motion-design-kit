...
mm.add("(prefers-reduced-motion: no-preference)", () => {
  const root = document.querySelector('[data-motion="pinned-hotspot-journey"]');
  if (!root) return;
  const items = gsap.utils.toArray('[data-motion="pinned-hotspot-journey"] [data-motion-item]');
...
mm.add("(prefers-reduced-motion: reduce)", () => {
  gsap.set("[data-motion="pinned-hotspot-journey"], [data-motion="pinned-hotspot-journey"] [data-motion-item]", { clearProps: "all" });
});
