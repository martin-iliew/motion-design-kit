...
Read the target file, `.claude/motion-library/scores.yaml`, and `.claude/motion-library/trends-overview.md`. The scores file gives you the pattern IDs per element type; the trends overview gives you the "how it should feel" and "when NOT to use" context for each pattern — use both to make informed pattern selections. Then:

1. Scan the file top-to-bottom, extract element types: nav, menu, hero, heading, badge, card, button, cta, section, footer, background, logos, pricing, stat, label, grid, tabs, accordion, modal, drawer, toast, tooltip, form, input, gallery, carousel, media, image, video, svg, icon, loader, timeline, progress-bar, hotspot
...
Use the selected pattern folders as the primary implementation source. For every winning pattern from `scores.yaml`, load that pattern's `spec.yaml` plus snippet from `.claude/motion-library/[pattern-id]/` first. Only fall back to `references/gsap-patterns.md` when the folder snippet is too narrow for the page composition or when you need a shared multi-element scaffold.

The inline pattern map below is fallback-only. NEVER substitute CSS keyframe classes when a GSAP pattern exists. Every element from Phase 1 must be implemented.
...
All token values are inlined below — **do not read `.claude/motion-tokens.md`** unless you need edge-case details (will-change policy, Tailwind mapping). Use these values directly.
...
1. Parse the source animation into a mental Motion Spec document (see `.claude/motion-spec.md` for schema)
...
- Output the full YAML spec document using the schema from `.claude/motion-spec.md`
...
| --------------------------------- | --------------------------------------------------------------------------------------------------- |
| Pattern selection index           | `.claude/motion-library/scores.yaml`                                                                |
| Trend context (feel, avoid)       | `.claude/motion-library/trends-overview.md`                                                         |
| GSAP production templates (expanded) | `references/gsap-patterns.md`                                                                    |
...
| Translation requested             | `.claude/motion-spec.md` + `references/motion-spec-translation-guide.md`                            |
| Token edge cases (will-change, Tailwind mapping) | `.claude/motion-tokens.md`                                              |
...
| Pattern selected from scores.yaml | Load only that pattern's `spec.yaml` + snippet from `.claude/motion-library/[pattern-id]/`         |
...
3. Load **only the winning pattern's `spec.yaml` + snippet** from `.claude/motion-library/[pattern-id]/`
